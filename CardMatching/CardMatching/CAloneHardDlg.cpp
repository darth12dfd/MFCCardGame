...
IMPLEMENT_DYNAMIC(CAloneHardDlg, CDialogEx)
	...
	: CDialogEx(IDD_ALONE_HARD_DIALOG, pParent)
{
	...
	for (int i = 0; i < 63; i++)
	{
		first_choice = rand() % 64;
		second_choice = rand() % 64;
...
BEGIN_MESSAGE_MAP(CAloneHardDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_TIMER()
	ON_WM_LBUTTONDOWN()
...
// CAloneHardDlg 메시지 처리기

// CCardMatchingDlg 메시지 처리기

BOOL CAloneHardDlg::OnInitDialog()
{
...
void CAloneHardDlg::OnPaint()
{
	...
	else
	{
		...
		for (int i = 0; i < 64; i++)
		{
			index = m_game_table[i] + 1; //index는 1~32
			...
			m_card_list[index].Draw(dc, (i % 8) * 60, (i / 8) * 80, 60, 80);
		}
	}
}
...
//  이 함수를 호출합니다.
HCURSOR CAloneHardDlg::OnQueryDragIcon()
{
...
void CAloneHardDlg::OnTimer(UINT_PTR nIDEvent)
{
...
void CAloneHardDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	if (m_front_back) return; //m_front_back 이 1이면 마우스 클릭을 못함

	if (point.x < 60 * 8 && point.y < 80 * 8)
	{
		int x = point.x / 60;
		int y = point.y / 80;
		int pos = y * 8 + x;
